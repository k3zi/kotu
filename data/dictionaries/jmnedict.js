    ...
    models.sequelize.sync().then(() => {
        function proccesEntry(entry, raw_entry) {
            return new Promise((resolve, reject) => {
                return Promise.all((raw_entry.k_ele || []).map(raw_k_ele => {
                    ...
                    return Promise.all((raw_entry.r_ele || []).map(raw_r_ele => {
                        const r_ele_data = {
                        ...
                        return Promise.all((raw_entry.trans || []).map(raw_trans => {
                            const trans_data = {
        ...
        Promise.mapSeries(dict, raw_entry => {
            return models.JMdictEntry.create({
                reference: {
                    'jmnedict_id': raw_entry.ent_seq[0]
                }
            }).then(entry => {
                return proccesEntry(entry, raw_entry);
            });
        }).then(lastEntry => {
